...
public class FileServiceImpl implements FileService {
    ...
    @Override
    public Map uploadWord(MultipartFile file) throws IOException {
        Map<String, String> map = storeFile(file, Paths.get(fileUploadPath, "word").toString());
        return map;
    }

    @Override
    public Map uploadFeedbackAttachment(MultipartFile file) throws IOException {
        Map<String, String> map = storeFile(file, Paths.get(fileUploadPath, "feedbackAttachment").toString());
        return map;
    }
    ...
    private static Map<String, String> storeFile(MultipartFile file, String fileUploadPath) throws IOException {
        ...
        try {
            ...
            if (fileUploadPath.contains("word")) {
                map.put("url", "./word" + relPath);
            } else if(fileUploadPath.contains("image")) {
                map.put("url", "./image" + relPath);
            }else if(fileUploadPath.contains("feedbackAttachment")){
                map.put("url", "./feedbackAttachment" + relPath);
            }
            log.info(relPath);
            return map;
